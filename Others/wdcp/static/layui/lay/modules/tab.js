...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            if (_config.maxSetting !== 'undefined') {
                ...
                if (typeof _config.maxSetting === 'number') {
                    if (currentTabCount === _config.maxSetting) {
                        ...
                        // 当超过maxSetting个时，关闭最先打开的选项卡
                        var tabs=$(_config.elem).find('ul.layui-tab-title').find('li');
                        var firstId=0;
                        tabs.each(function(){
                            firstId=$(this).attr("lay-id");
                            if(firstId!==undefined){
                                element.tabDelete(ELEM.tabFilter, firstId);
                                return false;
                            }
                        });
                        return;
            ...
            if (_config.closed) {
                title += '<i class="layui-icon iconfont icon-times layui-unselect layui-tab-close" data-id="' + globalTabIdIndex + '"></i>';
            }
            ...
            //当选项卡宽度大于浏览器宽度时，关闭最先打开的选项卡
            if(_config.maxWidth !== 'undefined'&& _config.maxWidth !== false ){
                var title=_config.elem.find('ul.layui-tab-title');
                //循环关闭最先打开的选项卡，直到选项卡宽度小于浏览器宽度
                while(title.prop('scrollWidth') > title.outerWidth()+1){
                    var tabs=$(_config.elem).find('ul.layui-tab-title').find('li');
                    var firstId=0;
                    tabs.each(function(){
                        firstId=$(this).attr("lay-id");
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                var ul = '<ul>';
                ul += '<li data-target="refresh" title="刷新当前选项卡"><i class="layui-icon iconfont icon-refresh" aria-hidden="true"></i> 刷新</li>';
                ul += '<li data-target="closeCurrent" title="关闭当前选项卡"><i class="layui-icon iconfont icon-times" aria-hidden="true"></i> 关闭当前</li>';
                ul += '<li data-target="closeOther" title="关闭其他选项卡"><i class="layui-icon iconfont icon-times-dot" aria-hidden="true"></i> 关闭其他</li>';
                ul += '<li data-target="closeAll" title="关闭全部选项卡"><i class="layui-icon iconfont icon-times-circle" aria-hidden="true"></i> 全部关闭</li>';
                ul += '</ul>';
                div.innerHTML = ul;
    ...
    //获取指定index的tabid
    Tab.prototype.getIndexTabId = function ( index ) {
        var that = this;
        var _config = that.config;
        return $(_config.elem).find('ul.layui-tab-title').find('li').eq( index ).attr('lay-id');
    }
    /**
	 * 获取当前获得焦点的tabid
	 */
