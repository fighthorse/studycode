  ...
  ,call = {
      ...
      if(parents.hasClass(NAV_TREE)){
        ...
        if(child.css("display")==='none'){
        	if(!child.is(":animated")){
                ...
                child.height(0);
                child.animate({"height":height+"px"});
                //关闭其他已打开菜单
            	$("."+NAV_ITEM+'ed').each(function(){
        ...
        //关闭菜单
        	...
        	if(!child.is(":animated")){
        		var height=child.height();
            	child.animate({"height":"0px"},400,'',function(){
    ...
    ,collapse: function(){
      ...
      if(typeof parents.attr('lay-accordion') === 'string'){
        var show = parents.children('.layui-colla-item').children('.collapse-show');
        show.slideUp('fast');
        show.parent().find('.layui-colla-title .layui-colla-icon').removeClass('icon-down-line').addClass('icon-up-line');
      }else{
    	  
      }
      if(isNone){
        elemCont.slideDown('fast');
      }else{
        elemCont.slideUp('fast');
      }
      elemCont[isNone ? 'addClass' : 'removeClass']('collapse-show');
      icon[isNone ? 'addClass' : 'removeClass']('icon-down-line');
      icon[isNone ? 'removeClass' : 'addClass']('icon-up-line');
            ...
            othis.on('mouseleave', function(){
              ...
              timeEnd[index] = setTimeout(function(){
                if(othis.hasClass(NAV_TREE)){
                  bar.css({
                    height: 0
                    ,top: bar.position().top + bar.width()/2
                    ,opacity: 0
                  });
                } else {
                  bar.css({
      ...
      ,collapse: function(){
        ...
        $('.'+ELEM).each(function(){
          ...
          elemItem.each(function(){
            ...
            elemTitle.append('<i class="layui-icon layui-colla-icon iconfont '+(isNone ? ' icon-up-line' : ' icon-down-line')+'"></i>');
