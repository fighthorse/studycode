...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.render = function(sets){
    ...
    var cols_count = options.cols[0].length ,//总列数 
    cols_width_sum = 0 , //已设置总列宽
    elem_width = options.width ? options.width :options.elem.parent().width();//美化元素
    var auto_cols=new Array();
    for(var cols_cur = 0 ; cols_cur < cols_count ; cols_cur ++){
      if(options.cols[0][cols_cur]['checkbox']===true){//判断复选框列
        cols_width_sum+=48;
      }else if(options.cols[0][cols_cur]['space']===true){//判断空隙列
        cols_width_sum+=15;
      }else if(options.cols[0][cols_cur]['width']==='auto'){//判断自动列宽
        auto_cols.push(cols_cur);
      }else{
        cols_width_sum+=parseInt(options.cols[0][cols_cur]['width'])?parseInt(options.cols[0][cols_cur]['width']):50;//默认
      }
    };
    var fixed_width=(elem_width-cols_width_sum-30)/auto_cols.length;
    for(var auto_col = 0 ; auto_col < auto_cols.length ; auto_col++){
      options.cols[0][auto_cols[auto_col]].width=fixed_width;
    }
